...
//         book.innerHTML = stage0_generateRandomBooks(stage0_booksArr[stage0_curIndex])[i];
//         book.id = `Stage0_Book${i}`;
//         book.onclick = () => stage0_showNextQuest(stage0_booksArr[i]);
//         document.getElementById("Stage0_BooksBar")?.appendChild(book);
//     }
// }

// const stage0_setCurrentSimptom = () => {
//     document.getElementById("Stage0_Simptoms")!.innerHTML = `${stage0_simptomsArr[stage0_curIndex]}`;
// }
...
// const stage0_generateRandomBooks = (rightAnswer: string) => {
//     let rr: string[] = [];
//     while (rr.length < 5) {
//         let i: number = Math.floor(Math.random()*stage0books.length);
//         if ( rr.indexOf(stage0books[i]) === -1) {
//             rr.push(stage0books[i]);
//         }
//     }
